...
- ÉCHEC (exit != 0) seulement si :
    ...
    * absence de docstring module ET absence de bannière de commentaires
      (⚠ sauf pour les fichiers dans `tests/` où c'est seulement une alerte)
...
for py in iter_py_files(root):
    ...
    # module docstring or banner? (fatal if none, except in tests/)
    mod_doc = ast.get_docstring(mod)
    has_banner = bool(mod_doc) or banner_in_comments(text)

    if "tests" in py.parts:
        if not has_banner:
            alerts.append(f"{py}: missing module docstring or banner (ignored for tests)")
    else:
        if not has_banner:
